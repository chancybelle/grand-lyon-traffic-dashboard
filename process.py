...
# ============================================================================
# 1. SAISIE SÉCURISÉE DES IDENTIFIANTS
# ============================================================================
MONGO_URI = os.getenv("MONGO_URI")

if not MONGO_URI:
    print("🔐 Connexion à la base de données MongoDB Atlas")
    MONGO_URI = getpass.getpass("Collez votre URI MongoDB (la saisie reste masquée) : ").strip()
    
    # Si getpass ne capture rien dans certains terminaux, fallback sur input classique
    if not MONGO_URI:
        MONGO_URI = input("🔑 Collez votre URI MongoDB : ").strip()

if not MONGO_URI:
    print("❌ L'URI MongoDB est obligatoire pour démarrer le pipeline.")
    sys.exit(1)
...
from pyspark.sql.functions import (
    col, from_json, expr, sha2, when, lit, struct, split, 
    upper, to_timestamp, current_timestamp, coalesce
)
...
df_clean = df_parsed.select(
    ...
    to_timestamp(col("last_update")).alias("horodatage_source"),
...
# ============================================================================
# 11. ÉCRITURE MONGODB (AVEC UPSERT / REMPLACEMENT)
# ============================================================================
def write_batch_sync(batch_df, batch_id):
    ...
    try:
        # Collection Standard (Mise à jour selon id_segment)
        batch_df.write \
            .format("mongodb") \
            ...
            .option("spark.mongodb.write.idFieldList", "id_segment") \
            .mode("append") \
            .save()

        # Collection Sécurisée (Mise à jour selon id_segment_secure)
        df_secure = batch_df \
        ...
        df_secure.write \
            ...
            .option("spark.mongodb.write.replaceDocument", "true") \
            .option("spark.mongodb.write.idFieldList", "id_segment_secure") \
            .mode("append") \
            .save()

        print(f"✅ Batch {batch_id} synchronisé avec succès !")
